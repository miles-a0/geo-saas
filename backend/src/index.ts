...
const startServer = async () => {
  try {
    ...
    try {
      ...
      logger.warn('Migration error (table may already exist):', migrationError);
    }

    try {
      const { seedDatabase } = await import('./db/migrations.js');
      await seedDatabase();
      logger.info('Database seeding completed');
    } catch (seedError) {
      logger.warn('Seed error (admin may already exist):', seedError);
    }
